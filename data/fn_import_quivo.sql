-- Constraints to support upserts
ALTER TABLE contacts
    ADD CONSTRAINT unique_workspace_external_code
        UNIQUE (workspace_id, external_code);

ALTER TABLE appointment_categories
    ADD CONSTRAINT unique_appointment_categories_external_code
        UNIQUE (workspace_id, name);

ALTER TABLE companies
    DROP CONSTRAINT unique_workspace_company;
ALTER TABLE companies
    ADD CONSTRAINT unique_workspace_company
        UNIQUE (workspace_id, name, address);

ALTER TABLE appointments
    DROP CONSTRAINT unique_appointments_external_code;
ALTER TABLE appointments
    ADD CONSTRAINT unique_appointments_external_code
        UNIQUE (workspace_id, contact_id, company_id, cind_id, appointment_category_id, job_title_category_id
            );
...
WHERE table_name = search_table_name
...
SELECT *
FROM import.reset_workspace(169);

-- 💣 Delete all data from specified workspace
CREATE
    OR REPLACE FUNCTION import.reset_workspace(search_workspace_id BIGINT) RETURNS VOID AS
$$
DECLARE
BEGIN
    delete
    from associations
    where workspace_id = search_workspace_id;
    ...
    delete
    from associative_status
    where workspace_id = search_workspace_id;

    delete
    from appointment_phones
    where workspace_id = search_workspace_id;
...
CREATE
    OR REPLACE FUNCTION import.run_import(associated_workspace_id BIGINT, table_nameska varchar) RETURNS VOID AS
$$
DECLARE
    rec           RECORD;
...
BEGIN
    FOR rec IN (SELECT id,
                    ...
                    import_row
                FROM import.import_data
                WHERE table_name = table_nameska
                ORDER BY
...
-- 📛 personal_titles record
            IF (rec.table_name = 'personal_titles') THEN
                INSERT INTO personal_titles (workspace_id, name)
                VALUES (associated_workspace_id,
                        rec.import_row ->> 'name')
                ON CONFLICT (workspace_id, name) DO NOTHING
                RETURNING id INTO updated_fk_id;

-- 🔠 cinds record
            ELSIF (rec.table_name = 'cinds') THEN
                INSERT INTO cinds (workspace_id, name, code)
                VALUES (associated_workspace_id,
                        rec.import_row ->> 'name',
                        rec.import_row ->> 'code')
                ON CONFLICT (workspace_id, name, code) DO NOTHING
                RETURNING id INTO updated_fk_id;

-- 📝 appointment_categories record
            ELSIF (rec.table_name = 'appointment_categories') THEN
                INSERT INTO appointment_categories (workspace_id, name, color)
                VALUES (associated_workspace_id,
                        rec.import_row ->> 'name',
...
-- 📖 contacts record
            ELSIF (rec.table_name = 'contacts') THEN
                ...
                ON CONFLICT (workspace_id, external_code) DO NOTHING
                RETURNING id INTO updated_fk_id;

-- 🧾 company_types record
            ELSIF (rec.table_name = 'company_types') THEN
                INSERT INTO company_types (workspace_id, name, description, examples)
                VALUES (associated_workspace_id,
                        rec.import_row ->> 'name',
...
-- 🏢 companies record
            ELSIF (rec.table_name = 'companies') THEN
                ...
                INTO updated_fk_id
                FROM companies
                WHERE workspace_id = associated_workspace_id
                AND name = rec.import_row ->> 'name'
                AND address = rec.import_row ->> 'address'
                LIMIT 1;
                IF updated_fk_id IS NULL THEN
                    ...
                    ON CONFLICT (workspace_id, name, address) DO NOTHING
                    RETURNING id INTO updated_fk_id;
                END IF;

-- 🏢 job_title_categories record
            ELSIF (rec.table_name = 'job_title_categories') THEN
                INSERT INTO job_title_categories (workspace_id, name, description)
                VALUES (associated_workspace_id,
                        rec.import_row ->> 'name',
                        rec.import_row ->> 'description')
                ON CONFLICT (workspace_id, name) DO NOTHING
            ...
            ELSIF (rec.table_name = 'appointments') THEN
                ...
                VALUES (associated_workspace_id,
                        ...
                        import.get_imported_record_fk(
                                ...
                                (rec.import_row ->> 'IMPORT_job_title_category_id') :: DECIMAL
                        ),
                        import.get_imported_record_fk(
                                'cinds',
                        ...
                        import.get_imported_record_fk(
                                ...
                                (rec.import_row ->> 'IMPORT_appointment_category_id') :: DECIMAL
                        ),
                        ...
                        rec.import_row ->> 'source')
                ON CONFLICT (
                    workspace_id, contact_id, company_id, cind_id, appointment_category_id, job_title_category_id
                    ) DO NOTHING
                RETURNING id INTO updated_fk_id;
--- 🏛️ associations record
            ELSIF (rec.table_name = 'associations') THEN
                INSERT INTO associations (workspace_id, name)
                VALUES (associated_workspace_id,
                        rec.import_row ->> 'name')
                ON CONFLICT (workspace_id, name) DO NOTHING
                RETURNING id INTO updated_fk_id;
...
-- ✉️ appointment_emails record
            ELSIF (rec.table_name = 'appointment_emails') THEN
                INSERT INTO appointment_emails (
                    appointment_id,
                    email,
                    is_preferred,
                    workspace_id
                    )
                VALUES (import.get_imported_record_fk(
                                'appointments',
                                (rec.import_row ->> 'IMPORT_appointment_id') :: DECIMAL
                        ),
                        rec.import_row ->> 'email',
                        COALESCE((rec.import_row ->> 'is_preferred')::boolean, false),
                        associated_workspace_id)
                RETURNING id INTO updated_fk_id;

--- appointment_phones record
            ELSIF (rec.table_name = 'appointment_phones') THEN
                INSERT INTO appointment_phones (
                    phone,
                    type,
                    workspace_id,
                    appointment_id
                    )
                VALUES (rec.import_row ->> 'phone',
                        rec.import_row ->> 'type',
                        associated_workspace_id,
                        import.get_imported_record_fk(
                                'appointments',
                ...
                RETURNING id INTO updated_fk_id;-- associative_status record

            ELSIF (rec.table_name = 'associative_status') THEN

                INSERT INTO associative_status (name, workspace_id)
                VALUES (rec.import_row ->> 'name',
                        associated_workspace_id)
                RETURNING id INTO updated_fk_id; --- company sections record

            ELSIF (rec.table_name = 'company_sections') THEN

                INSERT INTO company_sections (name, workspace_id)
                VALUES (rec.import_row ->> 'name',
                        associated_workspace_id)
                RETURNING id INTO updated_fk_id;
...
--- contact_statuses table
            ELSIF (rec.table_name = 'contact_statuses') THEN
                INSERT INTO contact_statuses (name, workspace_id)
                VALUES (rec.import_row ->> 'name',
                        associated_workspace_id)
                RETURNING id INTO updated_fk_id;
            ELSE
                -- 🤷‍♂️ Raise a warning if the record's table_name is undefined in this script.
                RAISE WARNING 'Undefined handler for table_name: "%"',
                    rec.table_name;
...
EXCEPTION
    WHEN OTHERS THEN RAISE EXCEPTION 'Error processing "%" record (id: %)\n%\n%',
        rec.table_name,
        rec.id,
...
SELECT *
FROM import.run_import(169, 'appointment_emails');

-- Check if all the records of the table have fk_id
SELECT DISTINCT table_name
FROM import.import_data
WHERE fk_id IS NULL;

-- Flush a table by workspace_id
DELETE
FROM appointments
WHERE workspace_id = 169;

-- Reset fk_id in the import table
UPDATE import.import_data
SET fk_id = null
WHERE table_name = 'appointment_emails';
