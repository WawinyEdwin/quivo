...
export const generate_filmpresa_ticket = async (
  ticketData: ITicketData
): Promise<Uint8Array> => {
  const doc = await PDFDocument.create();
  const page = doc.addPage();
  ...
  //   day: "numeric",
  // });
  const startDate = new Date(ticketData.event.date_start as string);
  const endDate = new Date(ticketData.event.date_end as string);
